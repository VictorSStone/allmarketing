...
@Directive({
  selector: `
  [AspectRatio],
  [AspectRatio.xs], [AspectRatio.sm], [AspectRatio.md], [AspectRatio.lg], [AspectRatio.xl],
  [AspectRatio.lt-sm], [AspectRatio.lt-md], [AspectRatio.lt-lg], [AspectRatio.lt-xl],
  [AspectRatio.gt-xs], [AspectRatio.gt-sm], [AspectRatio.gt-md], [AspectRatio.gt-lg],
  `
})
export class AspectRatioDirective extends BaseFxDirective implements OnInit, AfterContentInit, OnChanges, OnDestroy {
  ...
  constructor(monitor: MediaMonitor,
    ...
    this.renderer.addClass(this.pseudoElement, 'aspect-ratio-pseudo-element');

    this._cacheInput('aspectRatio', '');
  }

  /* tslint:disable */
  @Input('AspectRatio') set aspectRatio(val) {
    this._cacheInput('aspectRatio', val);
  };

  @Input('AspectRatio.xs') set aspectRatioXs(val) {
    this._cacheInput('aspectRatioXs', val);
  };

  @Input('AspectRatio.sm') set aspectRatioSm(val) {
    this._cacheInput('aspectRatioSm', val);
  };

  @Input('AspectRatio.md') set aspectRatioMd(val) {
    this._cacheInput('aspectRatioMd', val);
  };

  @Input('AspectRatio.lg') set aspectRatioLg(val) {
    this._cacheInput('aspectRatioLg', val);
  };

  @Input('AspectRatio.xl') set aspectRatioXl(val) {
    this._cacheInput('aspectRatioXl', val);
  };

  @Input('AspectRatio.gt-xs') set aspectRatioGtXs(val) {
    this._cacheInput('aspectRatioGtXs', val);
  };

  @Input('AspectRatio.gt-sm') set aspectRatioGtSm(val) {
    this._cacheInput('aspectRatioGtSm', val);
  };

  @Input('AspectRatio.gt-md') set aspectRatioGtMd(val) {
    this._cacheInput('aspectRatioGtMd', val);
  };

  @Input('AspectRatio.gt-lg') set aspectRatioGtLg(val) {
    this._cacheInput('aspectRatioGtLg', val);
  };

  @Input('AspectRatio.lt-sm') set aspectRatioLtSm(val) {
    this._cacheInput('aspectRatioLtSm', val);
  };

  /* tslint:enable */

  @Input('AspectRatio.lt-md') set aspectRatioLtMd(val) {
    this._cacheInput('aspectRatioLtMd', val);
  };

  @Input('AspectRatio.lt-lg') set aspectRatioLtLg(val) {
    this._cacheInput('aspectRatioLtLg', val);
  };

  @Input('AspectRatio.lt-xl') set aspectRatioLtXl(val) {
  ...
  ngAfterContentInit() {
    if (this.hostElement && this.pseudoElement && this.contentElement) {
      this.renderer.addClass(this.hostElement.nativeElement, 'aspect-ratio-host-element');
